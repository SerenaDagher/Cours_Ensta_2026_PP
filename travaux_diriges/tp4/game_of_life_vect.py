...
def compute_next_iteration(self):
    """
    Calcule la prochaine génération de cellules (grille torique)
    """
    c = self.cells

    # Compter les 8 voisins (tore) avec np.roll
    voisins = (
        np.roll(np.roll(c,  1, axis=0),  1, axis=1) +  # haut-gauche
        np.roll(c,  1, axis=0) +                      # haut
        np.roll(np.roll(c,  1, axis=0), -1, axis=1) + # haut-droite
        np.roll(c, -1, axis=0) +                      # bas
        np.roll(np.roll(c, -1, axis=0),  1, axis=1) + # bas-gauche
        np.roll(np.roll(c, -1, axis=0), -1, axis=1) + # bas-droite
        np.roll(c,  1, axis=1) +                      # gauche
        np.roll(c, -1, axis=1)                        # droite
    )

    # Règles du jeu
    next_cells = np.zeros_like(c, dtype=np.uint8)
    next_cells[(c == 1) & ((voisins == 2) | (voisins == 3))] = 1  # survie
    next_cells[(c == 0) & (voisins == 3)] = 1                     # naissance
